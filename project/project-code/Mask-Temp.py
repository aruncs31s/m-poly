...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if(mask < withoutMask):
            ...
            pygame.mixer.music.load("audio/temperature.mp3")
            pygame.mixer.music.play()
            while pygame.mixer.music.get_busy() == True:
                continue
            ...
            try:
                ...
                temperature = measure_body_temp()
                label = "Mask" 
                #color = (0, 255, 0) 
                #color1 = (0, 0, 255)
                
                label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)
                label1 = "Temp: " + str(temperature)

                cv2.putText(frame_copy, label1, (startX, startY - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color1, 2)
                cv2.putText(frame_copy, label, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
                cv2.rectangle(frame_copy, (startX, startY), (endX, endY), color, 2)
                ...
                if(temperature > 35.0):
                    ...
                    pygame.mixer.music.load("audio/high-temp.mp3")
                    pygame.mixer.music.play()
                    while pygame.mixer.music.get_busy() == True:
                        continue
                    ...
                    cv2.putText(frame_copy, label2, (startX, startY - 70), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color2, 2)
                    cv2.imshow("Frame", frame_copy)
                    key = cv2.waitKey(1) & 0xFF
                ...
                print("Please check your temperature sensor")
